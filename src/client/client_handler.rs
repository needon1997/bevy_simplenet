...
pub(crate) struct ClientHandler<Channel: ChannelPack> {
    ...
    pub(crate) client_event_sender: tokio::sync::mpsc::UnboundedSender<ClientEventFrom<Channel>>,
...
impl<Channel: ChannelPack> ezsockets::ClientExt for ClientHandler<Channel> {
    ...
    async fn on_text(&mut self, text: String) -> Result<(), ezsockets::Error> {
        match env_type() {
            ...
            EnvType::Wasm => {
                ...
                let Some((var, value)) = text.as_str().split_once(':') else {
                    tracing::warn!("ignoring invalid text from server...");
                    return Ok(());
                };

                // try to deserialize timestamp
                let Ok(timestamp) = u128::from_str_radix(value, 10u32) else {
    ...
    async fn on_binary(&mut self, bytes: Vec<u8>) -> Result<(), ezsockets::Error> {
        ...
        // deserialize message
        let Ok(server_msg) = bincode::DefaultOptions::new().deserialize(&bytes[..]) else {
        ...
        let client_event = match server_msg {
            ...
            ClientMetaEventFrom::<Channel>::Response(response, request_id) => {
                ...
                let Ok(mut pending_requests) = self.pending_requests.lock() else {
                    tracing::error!("Response lock error");
                    return Ok(());
                };
            ...
            ClientMetaEventFrom::<Channel>::Reject(request_id) => {
                ...
                // - this should never happen
                let Ok(mut pending_requests) = self.pending_requests.lock() else {
                    tracing::error!("Reject lock error");
                    return Ok(());
                };
    ...
    async fn on_connect(&mut self) -> Result<(), ezsockets::Error> {
        ...
        let Ok(mut pending_requests) = self.pending_requests.lock() else {
            tracing::error!("on_connect() lock error");
            return Ok(());
        };
        ...
        let aborted_sends =
            Self::final_request_cleanup(&mut pending_requests, &self.client_event_sender);
        ...
        for aborted_send in aborted_sends {
            if let Err(err) = self
                .client_event_sender
                .send(ClientEventFrom::<Channel>::SendFailed(aborted_send))
            {
        ...
        if let Err(err) = self
            .client_event_sender
            .send(ClientEventFrom::<Channel>::Report(ClientReport::Connected))
        {
    ...
    async fn on_connect_fail(
        ...
        // lock the pending requests cache
        let Ok(mut pending_requests) = self.pending_requests.lock() else {
            tracing::error!("on_connect_fail() lock error");
    ...
    async fn on_disconnect(
        ...
        // lock the pending requests cache
        let Ok(mut pending_requests) = self.pending_requests.lock() else {
            tracing::error!("on_disconnect() lock error");
            return Ok(ezsockets::client::ClientCloseMode::Close);
        };

        // mark the client as disconnected
        // - We do this within the pending requests lock in order to synchronize with the client API.
        self.client_disconnected_count
            .fetch_add(1u16, Ordering::Release);

        // forward event to client owner
        if let Err(err) = self
            .client_event_sender
            .send(ClientEventFrom::<Channel>::Report(
    ...
    async fn on_close(
        ...
        // lock the pending requests cache
        let Ok(mut pending_requests) = self.pending_requests.lock() else {
            tracing::error!("on_close() lock error");
            return Ok(ezsockets::client::ClientCloseMode::Close);
        };
        ...
        if let Err(err) = self
            .client_event_sender
            .send(ClientEventFrom::<Channel>::Report(
                ClientReport::ClosedByServer(close_frame),
            ))
        {
...
impl<Channel: ChannelPack> ClientHandler<Channel> {
    fn clean_pending_requests(
        ...
        client_event_sender: &tokio::sync::mpsc::UnboundedSender<ClientEventFrom<Channel>>,
    ) {
        for failed_req in pending_requests.drain_failed_requests() {
            match failed_req.status() {
                RequestStatus::SendFailed => {
                    if let Err(err) = client_event_sender
                        .send(ClientEventFrom::<Channel>::SendFailed(failed_req.id()))
                    {
                ...
                RequestStatus::ResponseLost => {
                    if let Err(err) = client_event_sender
                        .send(ClientEventFrom::<Channel>::ResponseLost(failed_req.id()))
                    {
                        tracing::debug!(?err, "failed to forward client report to client");
                    }
                }
                status => {
                    tracing::error!(
                        ?status,
                        "unexpected request status while draining failed requests"
                    );
                }
            }
        }
...
impl<Channel: ChannelPack> Drop for ClientHandler<Channel> {
    fn drop(&mut self) {
        ...
        if let Err(err) = self
            .client_event_sender
            .send(ClientEventFrom::<Channel>::Report(ClientReport::IsDead(
                aborted_reqs,
            )))
        {
