...
pub struct Client<Channel: ChannelPack> {
    ...
    client_event_receiver: tokio::sync::mpsc::UnboundedReceiver<ClientEventFrom<Channel>>,
...
impl<Channel: ChannelPack> Client<Channel> {
    ...
    pub fn send(&self, msg: Channel::ClientMsg) -> MessageSignal {
        ...
        let Ok(_pending_requests) = self.pending_requests.lock() else {
        ...
        let Ok(ser_msg) =
            bincode::DefaultOptions::new().serialize(&ServerMetaEventFrom::<Channel>::Msg(msg))
        else {
    ...
    pub fn request(&self, request: Channel::ClientRequest) -> RequestSignal {
        ...
        if !self.is_connected() {
            ...
            return pending_requests
                .add_request(request_id, MessageSignal::new(MessageStatus::Failed));
        };
    ...
    pub fn try_next(&mut self) -> Option<ClientEventFrom<Channel>> {
        let Ok(msg) = self.client_event_receiver.try_recv() else {
            return None;
        };
        ...
        if let ClientEventFrom::<Channel>::Report(ClientReport::Connected) = &msg {
            self.client_disconnected_count
                .fetch_sub(1u16, Ordering::Release);
        }

        Some(msg)
    }

    pub async fn next(&mut self) -> Option<ClientEventFrom<Channel>> {
        let msg = self.client_event_receiver.recv().await?;
    ...
    pub fn close(&self) {
        // sanity check
        if self.is_closed() {
            tracing::warn!("tried to close an already closed client");
            return;
        }
        tracing::info!("client closing self");

        // close the client
        let closure_frame = ezsockets::CloseFrame {
...
impl<Channel: ChannelPack> ClientFactory<Channel> {
    ...
    pub fn new_client(
        ...
        let (client_event_sender, client_event_receiver) =
            tokio::sync::mpsc::unbounded_channel::<ClientEventFrom<Channel>>();
        ...
        let (client, _client_task_handle) = ezsockets::connect_with(
            move |client| ClientHandler::<Channel> {
                config,
                auth,
                client,
