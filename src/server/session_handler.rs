...
pub(crate) struct SessionHandler<Channel: ChannelPack> {
    ...
    pub(crate) server_event_sender:
        tokio::sync::mpsc::UnboundedSender<ClientSourceMsg<ClientId, ServerEventFrom<Channel>>>,
...
impl<Channel: ChannelPack> ezsockets::SessionExt for SessionHandler<Channel> {
    ...
    async fn on_text(&mut self, text: String) -> Result<(), ezsockets::Error> {
        match self.env_type {
            EnvType::Native => {
                ...
                tracing::trace!(
                    "received text from native client (not implemented), closing session..."
                );
                self.close("text not allowed");
                return Ok(());
            }
            EnvType::Wasm => {
                ...
                let Some((var, value)) = text.as_str().split_once(':') else {
                    ...
                    self.close("only ping/pong text allowed");
                    return Ok(());
                };
                ...
                let Ok(timestamp) = u128::from_str_radix(value, 10u32) else {
                    tracing::trace!(
                        "received invalid ping/pong timestamp from WASM client, closing session..."
                    );
                    self.close("only timestamp ping/pong allowed");
                    return Ok(());
                };
    ...
    async fn on_binary(&mut self, bytes: Vec<u8>) -> Result<(), ezsockets::Error> {
        ...
        if !self.rate_limit_tracker.try_count_msg() {
            ...
            self.close("rate limit violation");
            return Ok(());
        }
        ...
        if bytes.len() > self.max_msg_size as usize {
            ...
            self.close("message size violation");
            return Ok(());
        }
        let Ok(message) = bincode::DefaultOptions::new().deserialize(&bytes[..]) else {
            tracing::trace!(
                "received client message that failed to deserialize, closing session..."
            );
            self.close("deserialization failure");
            return Ok(());
        };
        ...
        match message {
            ServerMetaEventFrom::<Channel>::Authenticate(msg) => {
                ...
                if self.client_id.is_some() {
                    self.close("extra auth message");
                    return Ok(());
                }

                // authenticate the connection
                if !self.authenticator.authenticate(&msg.auth) {
                    self.close("invalid auth");
                    return Ok(());
                }
            ...
            ServerMetaEventFrom::<Channel>::Msg(msg) => {
                ...
                if let Err(err) = self.server_event_sender.send(ClientSourceMsg::new(
                    client_id,
                    ServerEventFrom::<Channel>::Msg(msg),
                )) {
            ...
            ServerMetaEventFrom::<Channel>::Request(request, request_id) => {
                // disconnect client if not fully authenticated yet
                let Some(client_id) = self.client_id else {
                    self.close("request before auth");
                    return Ok(());
                };
                ...
                if let Err(err) = self.server_event_sender.send(ClientSourceMsg::new(
                    client_id,
                    ServerEventFrom::<Channel>::Request(token, request),
                )) {
                    tracing::debug!(?err, "client msg sender is broken, closing session...");
                    self.close("session error");
                    return Ok(());
    ...
    async fn on_call(&mut self, _msg: ()) -> Result<(), ezsockets::Error> {
        tracing::info!(
            self.id,
            "received call (not implemented), closing session..."
        );
        self.close("session error");
        return Ok(());
    }
...
impl<Channel: ChannelPack> SessionHandler<Channel> {
    ...
    fn close(&mut self, reason: &str) {
        ...
        if let Err(_) = self.session.close(Some(ezsockets::CloseFrame {
