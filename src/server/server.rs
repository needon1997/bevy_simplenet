...
async fn websocket_handler<Channel: ChannelPack>(
    ...
    match prevalidate_connection_request(ezsocket_upgrade.request(), &pending, &count, &preval) {
        Ok(client_env_type) => {
            ezsocket_upgrade.on_upgrade_with_config(server, socket_config(&preval, client_env_type))
        }
...
async fn run_server(
    ...
    let server = match acceptor_config {
        ...
        AcceptorConfig::Rustls(config) => {
            server.acceptor(axum_server::tls_rustls::RustlsAcceptor::new(config))
        }
        #[cfg(feature = "tls-openssl")]
        AcceptorConfig::OpenSSL(config) => {
            server.acceptor(axum_server::tls_openssl::OpenSSLAcceptor::new(config))
        }
    };
...
pub struct Server<Channel: ChannelPack> {
    ...
    client_event_sender:
        tokio::sync::mpsc::UnboundedSender<ClientTargetMsg<ClientId, SessionCommand<Channel>>>,
    /// Receives server events from the internal connection handler.
    server_event_receiver:
        tokio::sync::mpsc::UnboundedReceiver<ClientSourceMsg<ClientId, ServerEventFrom<Channel>>>,
...
impl<Channel: ChannelPack> Server<Channel> {
    ...
    pub fn send(&self, id: ClientId, msg: Channel::ServerMsg) {
        if self.is_dead() {
            tracing::warn!(id, "tried to send message to client but server is dead");
            return;
        }
    ...
    pub fn respond(&self, token: RequestToken, response: Channel::ServerResponse) {
        ...
        if self.is_dead() {
            tracing::warn!(
                client_id,
                request_id,
                "tried to send response to session but server is dead"
            );
            return;
        }
        ...
        if token.destination_is_dead() {
            tracing::debug!(
                client_id,
                request_id,
                "tried to send response to dead session"
            );
            return;
        }
    ...
    pub fn ack(&self, token: RequestToken) {
        ...
        if self.is_dead() {
            tracing::warn!(
                client_id,
                request_id,
                "tried to send ack to session but server is dead"
            );
            return;
        }
        ...
        if let Err(err) = self.client_event_sender.send(ClientTargetMsg::new(
            client_id,
            SessionCommand::<Channel>::Send(
                ClientMetaEvent::Ack(request_id),
                None,
                Some(death_signal),
            ),
        )) {
    ...
    pub fn try_next(&mut self) -> Option<(ClientId, ServerEventFrom<Channel>)> {
        let Ok(ClientSourceMsg { id, msg }) = self.server_event_receiver.try_recv() else {
            return None;
        };
    ...
    pub async fn next(&mut self) -> Option<(ClientId, ServerEventFrom<Channel>)> {
        let ClientSourceMsg { id, msg } = self.server_event_receiver.recv().await?;
...
impl<Channel: ChannelPack> ServerFactory<Channel> {
        ...
        A: std::net::ToSocketAddrs + Send + 'static,
    {
        ...
        let (server_event_sender, server_event_receiver) = tokio::sync::mpsc::unbounded_channel::<
        ...
        let (server, server_worker) = enfync::blocking::extract(runtime_handle.spawn(async move {
            ezsockets::Server::create(move |server| ConnectionHandler::<Channel> {
        ...
        let server_closed_signal = runtime_handle.spawn(async move {
        ...
        let server_running_signal = runtime_handle
            .spawn(async move { run_server(app, connection_listener, acceptor_config).await });
