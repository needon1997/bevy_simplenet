...
fn reject_client_request<Channel: ChannelPack>(
    ...
    let Ok(ser_msg) = bincode::DefaultOptions::new().serialize(&packed_msg) else {
        tracing::error!(session_id, "serializing rejection failed");
        return;
    };
    ...
    if let Err(_) = session.binary(ser_msg) {
        tracing::debug!(
            session_id,
            "dropping request rejection sent to broken session"
        );
    }
}
...
impl<Channel: ChannelPack> ezsockets::ServerExt for ConnectionHandler<Channel> {
    ...
    async fn on_connect(
        ...
        if self.session_registry.len()
            >= (self.config.max_connections + self.config.max_pending) as usize
        {
        ...
        let session = ezsockets::Session::create(
            move |session| {
                ...
                let request_rejector = move |request_id: u64| {
    ...
    async fn on_disconnect(
        ...
        let Some(client_id) = self.session_to_client.remove(&id) else {
        ...
        if let Err(err) = self.server_event_sender.send(ClientSourceMsg::new(
            client_id,
            ServerEventFrom::<Channel>::Report(report),
        )) {
    ...
    async fn on_call(
        ...
        if let SessionCommand::<Channel>::Add {
            session_id,
            msg,
            env_type,
        } = client_msg.msg
        {
            let Some(session) = self.session_registry.get(&session_id) else {
            ...
            if self.client_to_session.contains_key(&client_msg.id) {
                let _ = session.close(Some(ezsockets::CloseFrame {
                    code: ezsockets::CloseCode::Policy,
            ...
            self.client_to_session
                .insert(client_msg.id, (session_id, self.total_connections_count));
        ...
        let Some((session_id, connection_idx)) = self.client_to_session.get(&client_msg.id) else {
            ...
            SessionCommand::<Channel>::Send(
                ...
                if let Some(death_signal) = maybe_death_signal {
                    if death_signal.is_dead() {
                        tracing::debug!(
                            client_msg.id,
                            "dropping response targeted at dead session"
                        );
                        return Ok(());
                    }
                }
                ...
                let Ok(ser_msg) = bincode::DefaultOptions::new().serialize(&msg_to_send) else {
                    tracing::error!(client_msg.id, "serializing message failed");
                    return Ok(());
                };
