...
fn request_response() {
    ...
    */
    ...
    let Some((msg_client_id, DemoServerEvent::Request(token, DemoClientRequest(msg_client_val)))) =
        websocket_server.try_next()
    else {
    ...
    let Some(DemoClientEvent::Response(DemoServerResponse(msg_server_val), request_id)) =
        websocket_client.try_next()
    else {
        panic!("client did not receive server msg");
    };
    ...
    let None = websocket_client.try_next() else {
        panic!("client should receive no more values");
    };
}
...
fn request_ack() {
    ...
    */
    ...
    let Some(DemoClientEvent::Ack(request_id)) = websocket_client.try_next() else {
        panic!("client did not receive server msg");
    };
    assert_eq!(signal.id(), request_id);
...
fn request_rejected() {
    ...
    */
    ...
    let Some(DemoClientEvent::Reject(request_id)) = websocket_client.try_next() else {
        panic!("client did not receive server msg");
    };
    assert_eq!(signal.id(), request_id);
...
fn request_dropped() {
    ...
    */
    ...
    let Some((dc_client_id, DemoServerEvent::Report(DemoServerReport::Disconnected))) =
        ...
        panic!("server should be disconnected after client is disconnected (by server)");
    };
    ...
    let Some(DemoClientEvent::ResponseLost(request_id)) = websocket_client.try_next() else {
        panic!("client did not receive server msg");
    };
    assert_eq!(signal.id(), request_id);

    // client receives connection report
    let Some((_, DemoServerEvent::Report(DemoServerReport::Connected(_, ())))) =
        websocket_server.try_next()
    else {
        panic!("server should be connected once client is connected");
    };
