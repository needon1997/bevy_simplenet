...
fn hello_world() {
    ...
    */
    ...
    let Some(DemoClientEvent::Msg(DemoServerMsg(msg_server_val))) = websocket_client.try_next()
    else {
    ...
    let Some((dc_client_id, DemoServerEvent::Report(DemoServerReport::Disconnected))) =
        ...
        panic!("server should be disconnected after client is disconnected (by client)");
    };
    let Some(DemoClientEvent::Report(bevy_simplenet::ClientReport::ClosedBySelf)) =
        websocket_client.try_next()
    else {
        panic!("client should have closed itself");
    };
