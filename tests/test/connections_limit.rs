...
fn connections_limit_test(max_connections: u32) {
    ...
    for client_num in 0..max_connections {
        ...
        let Some(DemoClientEvent::Report(bevy_simplenet::ClientReport::Connected)) =
            websocket_client.try_next()
        else {
            panic!("client should be connected to server");
        };
        let Some((_, DemoServerEvent::Report(DemoServerReport::Connected(_, _)))) =
            websocket_server.try_next()
        else {
            panic!("server should be connected to client: {}", client_num);
        };
    ...
    let Some(DemoClientEvent::Report(bevy_simplenet::ClientReport::IsDead(_))) =
        ...
        panic!("client should have failed to connect");
    };
    let None = websocket_server.try_next() else {
        panic!("server should not connect to another client");
    };
    assert_eq!(websocket_server.num_connections(), max_connections as u64);

    // 3. disconnect one client
    let mut client_to_disconnect = clients
        .pop()
        .expect("there should be at least one connected client");
    client_to_disconnect.close();

    std::thread::sleep(std::time::Duration::from_millis(100)); //wait for async machinery

    let Some(DemoClientEvent::Report(bevy_simplenet::ClientReport::ClosedBySelf)) =
        client_to_disconnect.try_next()
    else {
        panic!("client should be closed by self");
    };
    let Some((_, DemoServerEvent::Report(DemoServerReport::Disconnected))) =
        websocket_server.try_next()
    else {
        panic!("server should see a disconnected client");
    };
    assert_eq!(
        websocket_server.num_connections(),
        (max_connections - 1) as u64
    );
    ...
    let Some((_, DemoServerEvent::Report(DemoServerReport::Connected(_, _)))) =
        ...
        panic!("server should be connected to client");
    };
    assert_eq!(websocket_server.num_connections(), max_connections as u64);
    ...
    let None = websocket_client.try_next() else {
